...
data_out <- "~/GES/data07.RData"

if (!file.exists(data_out)) {
  acc_07 <- read.sas7bdat(paste0(data_base_dir, "GES07/GES2007/accident.sas7bdat"))
  acc07_sel <- acc_07[,c('CASENUM','YEAR','MONTH','WEEKDAY','HOUR_I', 'MINUTE_I','VEH_INVL','NON_INVL','MANCOL_I','INT_HWY',
                         'REL_JCT','REL_RWY','NUM_LAN','ALIGN','PROFILE','SUR_COND','TRAF_CON','SPD_LIM',
                         'LGHT_CON','SCHL_BUS','WEIGHT', 'STRATUM','PSU','PSUSTRAT')]
  
  acc07_sel$CASENUM <- as.character(acc07_sel$CASENUM)
  
  veh_07 <- read.sas7bdat(paste0(data_base_dir, "GES07/GES2007/vehicle.sas7bdat"))
  veh07_sel <- veh_07[,c('CASENUM','MODEL_YR','VIN','REGION','MAKE','BODY_TYP','SPEED','P_CRASH1',
                         'P_CRASH2','VEHNO','DR_DSTRD','SPEEDREL')]
  
  veh07_sel$CASENUM <- as.character(veh07_sel$CASENUM)
  veh07_sel$VEHNO <- as.character(veh07_sel$VEHNO)
  
  per_07 <- read.sas7bdat(paste0(data_base_dir, "GES07/GES2007/person.sas7bdat"))
  per07_sel <- per_07[,c('CASENUM','PERNO','PER_TYPE','VEHNO','SEX','AGE','INJ_SEV','PER_ALCH', 'PER_DRUG','IMPAIRMT','ACTION')]
  per07_sel <- per07_sel %>% filter(PER_TYPE %in% c(1,3,4,5,6,7,8))
  
  per07_sel$CASENUM <- as.character(per07_sel$CASENUM)
  per07_sel$VEHNO <- as.character(per07_sel$VEHNO)
  
  dis_07 <- read.sas7bdat(paste0(data_base_dir, "GES07/GES2007/distract.sas7bdat"))
  dis07_sel <- dis_07[,c('CASENUM','VEHNO','MDRDSTRD')]
  
  dis07_sel$CASENUM <- as.character(dis07_sel$CASENUM)
  dis07_sel$VEHNO <- as.character(dis07_sel$VEHNO)
  
  vis_07 <- read.sas7bdat(paste0(data_base_dir, "GES07/GES2007/vision.sas7bdat"))
  vis07_sel <- vis_07[,c('CASENUM','VEHNO','MVISOBSC')]
  
  vis07_sel$CASENUM <- as.character(vis07_sel$CASENUM)
  vis07_sel$VEHNO <- as.character(vis07_sel$VEHNO)
  
  dat1 <- inner_join(veh07_sel, dis07_sel, by = c('CASENUM', 'VEHNO'))
  dat1 <- inner_join(dat1, vis07_sel, by = c('CASENUM', 'VEHNO'))
  dat1 <- left_join(per07_sel, dat1, by = c('CASENUM', 'VEHNO'))
  ...
  dat_bus$bus_age <- 2007 - dat_bus$MODEL_YR
  ...
  acc07_sel <- data.table(acc07_sel)
  dat07 <- inner_join(dat3, acc07_sel, by = c('CASENUM'))
  save(dat07, file = data_out)
}
