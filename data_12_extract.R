...
data_out <- "~/GES/data12.RData"

if (!file.exists(data_out)) {
  acc_12 <- read.sas7bdat(paste0(data_base_dir, "GES12/GES12PCSAS/accident.sas7bdat"))
  acc12_sel <- acc_12[,c('CASENUM','YEAR','MONTH','DAY_WEEK','HOUR', 'MINUTE', 'PERNOTMVIT','VE_TOTAL','MAN_COLL','INT_HWY',
                         'RELJCT2','REL_ROAD','LGT_COND', 'WEATHER1','SCH_BUS','NUM_INJ',
                         'WEIGHT', 'STRATUM','PSU','PSUSTRAT')]
  
  acc12_sel$CASENUM <- as.character(acc12_sel$CASENUM)
  
  veh_12 <- read.sas7bdat(paste0(data_base_dir, "GES12/GES12PCSAS/vehicle.sas7bdat"))
  veh12_sel <- veh_12[, c('CASENUM','MOD_YEAR','BODY_TYP','TRAV_SP','P_CRASH1','P_CRASH2','VEH_NO','SPEEDREL', 'VNUM_LAN','VALIGN','VPROFILE','VSURCOND',
                          'VTRAFCON','VSPD_LIM')]
  veh12_sel$CASENUM <- as.character(veh12_sel$CASENUM)
  veh12_sel$VEH_NO <- as.character(veh12_sel$VEH_NO)
  
  per_12 <- read.sas7bdat(paste0(data_base_dir, "GES12/GES12PCSAS/person.sas7bdat"))
  per12_sel <- per_12[,c('CASENUM','PER_NO','PER_TYP','VEH_NO','SEX','AGE','INJ_SEV','DRINKING', 'DRUGS')]
  per12_sel <- per12_sel %>% filter(PER_TYP %in% c(1,3,4,5,6,7,8))
  
  per12_sel$CASENUM <- as.character(per12_sel$CASENUM)
  per12_sel$VEH_NO <- as.character(per12_sel$VEH_NO)
  
  imp_12 <- read.sas7bdat(paste0(data_base_dir, "GES12/GES12PCSAS/nmimpair.sas7bdat"))
  nmimp12_sel <- imp_12[,c('CASENUM','VEH_NO','NMIMPAIR')]
  
  nmimp12_sel$CASENUM <- as.character(nmimp12_sel$CASENUM)
  nmimp12_sel$VEH_NO <- as.character(nmimp12_sel$VEH_NO)
  
  dimp_12 <- read.sas7bdat(paste0(data_base_dir, "GES12/GES12PCSAS/drimpair.sas7bdat"))
  drimp12_sel <- dimp_12[,c('CASENUM','VEH_NO','DRIMPAIR')]
  
  drimp12_sel$CASENUM <- as.character(drimp12_sel$CASENUM)
  drimp12_sel$VEH_NO <- as.character(drimp12_sel$VEH_NO)
  
  dis_12 <- read.sas7bdat(paste0(data_base_dir, "GES12/GES12PCSAS/distract.sas7bdat"))
  dis12_sel <- dis_12[,c('CASENUM','VEH_NO','MDRDSTRD')]
  
  dis12_sel$CASENUM <- as.character(dis12_sel$CASENUM)
  dis12_sel$VEH_NO <- as.character(dis12_sel$VEH_NO)
  
  vis_12 <- read.sas7bdat(paste0(data_base_dir, "GES12/GES12PCSAS/vision.sas7bdat"))
  vis12_sel <- vis_12[,c('CASENUM','VEH_NO','MVISOBSC')]
  
  vis12_sel$CASENUM <- as.character(vis12_sel$CASENUM)
  vis12_sel$VEH_NO <- as.character(vis12_sel$VEH_NO)
  
  dat1 <- inner_join(veh12_sel, dis12_sel, by = c('CASENUM', 'VEH_NO'))
  dat1 <- inner_join(dat1, vis12_sel, by = c('CASENUM', 'VEH_NO'))
  dat1 <- inner_join(dat1, drimp12_sel, by = c('CASENUM', 'VEH_NO'))
  dat1 <- left_join(per12_sel, dat1, by = c('CASENUM', 'VEH_NO'))
  dat1 <- left_join(dat1, nmimp12_sel, by = c('CASENUM', 'VEH_NO'))
  ...
  dat_bus$bus_age <- 2012 - dat_bus$MOD_YEAR
  ...
  acc12_sel <- data.table(acc12_sel)
  dat12 <- inner_join(dat3, acc12_sel, by = c('CASENUM'))
  save(dat12, file = data_out)
}