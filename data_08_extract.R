...
data_out <- "~/GES/data08.RData"

if (!file.exists(data_out)) {
  acc_08 <- read.sas7bdat(paste0(data_base_dir, "GES08/GES2008/accident.sas7bdat"))
  acc08_sel <- acc_08[,c('CASENUM','YEAR','MONTH','WEEKDAY','HOUR_I', 'MINUTE_I','VEH_INVL','NON_INVL','MANCOL_I','INT_HWY',
                         'REL_JCT','REL_RWY','NUM_LAN','ALIGN','PROFILE','SUR_COND','TRAF_CON','SPD_LIM',
                         'LGHT_CON','SCHL_BUS','WEIGHT', 'STRATUM','PSU','PSUSTRAT')]
  
  acc08_sel$CASENUM <- as.character(acc08_sel$CASENUM)
  
  veh_08 <- read.sas7bdat(paste0(data_base_dir, "GES08/GES2008/vehicle.sas7bdat"))
  veh08_sel <- veh_08[,c('CASENUM','MODEL_YR','VIN','REGION','MAKE','BODY_TYP','SPEED','P_CRASH1',
                         'P_CRASH2','VEHNO','DR_DSTRD','SPEEDREL')]
  veh08_sel$CASENUM <- as.character(veh08_sel$CASENUM)
  veh08_sel$VEHNO <- as.character(veh08_sel$VEHNO)
  
  per_08 <- read.sas7bdat(paste0(data_base_dir, "GES08/GES2008/person.sas7bdat"))
  per08_sel <- per_08[,c('CASENUM','PERNO','PER_TYPE','VEHNO','SEX','AGE','INJ_SEV','PER_ALCH', 'PER_DRUG','IMPAIRMT','ACTION')]
  per08_sel$CASENUM <- as.character(per08_sel$CASENUM)
  per08_sel$VEHNO <- as.character(per08_sel$VEHNO)
  per08_sel <- per08_sel %>% filter(PER_TYPE %in% c(1,3,4,5,6,7,8))
  
  dis_08 <- read.sas7bdat(paste0(data_base_dir, "GES08/GES2008/distract.sas7bdat"))
  dis08_sel <- dis_08[,c('CASENUM','VEHNO','MDRDSTRD')]
  dis08_sel$CASENUM <- as.character(dis08_sel$CASENUM)
  dis08_sel$VEHNO <- as.character(dis08_sel$VEHNO)
  
  vis_08 <- read.sas7bdat(paste0(data_base_dir, "GES08/GES2008/vision.sas7bdat"))
  vis08_sel <- vis_08[,c('CASENUM','VEHNO','MVISOBSC')]
  
  vis08_sel$CASENUM <- as.character(vis08_sel$CASENUM)
  vis08_sel$VEHNO <- as.character(vis08_sel$VEHNO)
  
  dat1 <- inner_join(veh08_sel, dis08_sel, by = c('CASENUM', 'VEHNO'))
  dat1 <- inner_join(dat1, vis08_sel, by = c('CASENUM', 'VEHNO'))
  dat1 <- left_join(per08_sel, dat1, by = c('CASENUM', 'VEHNO'))
  ...
  dat_bus$bus_age <- 2008 - dat_bus$MODEL_YR
  ...
  acc08_sel <- data.table(acc08_sel)
  dat08 <- inner_join(dat3, acc08_sel, by = c('CASENUM'))
  save(dat08, file = data_out)
}
